<?php
	...
	if ($fullwidth != '1' && $nospacing == '1') { $css_class .= ' nospacing'; }
	if ($parallax == '1'){ $css_class .= ' parallax'; }
	if ($overlay == 'ewf-row-overlay-pattern' && $ewf_use_overlay == true){ $css_class .= ' row-overlay'; }
	
	
	
	$output .= '<div class="'.$css_class.'"'.$style.'>';
	
	if ($overlay == 'ewf-row-overlay-pattern' && $ewf_use_overlay == true){
	...
	if ($fullwidth == '1'){
		...
		$css_full_extra = null;
		if ($nospacing == '1'){
			$css_full_extra = ' nospacing';
		}
		
		$output = '<div class="ewf-full-width-section'.$css_full_extra.'">'.$output.'</div>';
	}
