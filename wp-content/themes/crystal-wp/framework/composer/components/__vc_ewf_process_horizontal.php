<?php
	...
	function ewf_vc_process_horizontal_group( $atts, $content ) {
		...
		return '<div class="horizontal-process-builder'.$class_extra.'"><ul class="fixed">'.do_shortcode($content).'</ul></div>';
	}
	...
	function ewf_vc_process_horizontal_item( $atts, $content ) {
		...
		echo '<li'.$class_extra.'>';
			...
			if ($icon){
				echo '<span><i '.$style_icon_color.'class="'.$icon.'"></i></span>';
			}
			
			echo '<div class="process-description">';
				echo '<h5'.$style_title_color.'>'.$title.'</h5> ';
				echo '<p>'.$content.'</p>';
