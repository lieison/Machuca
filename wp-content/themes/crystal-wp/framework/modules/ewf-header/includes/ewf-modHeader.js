	...
	jQuery(document).ready(function($) {
		...
		$('.mh-field-icon .ewf-mh-icon-action').click(function(){
			var field = $(this).closest('.mh-field-icon');
		
			if ($(field).hasClass('disabled')){
				return false;
			}
		
			if ($(this).hasClass('mh-icon-remove')){
				$('.ewf-mh-icon-preview i', field).attr('class', '');
				$('.ewf-mh-icon-preview span', field).html('');
				$('input' ,field).val('');
				
				$(this).removeClass('mh-icon-remove').addClass('mh-icon-set');
				
				return false;
			}
		
			if ( $('.ewf-mh-icon-list:visible', field).length ){
				$('.ewf-mh-icon-list:visible', field).slideUp('fast').addClass('mh-icon-remove');
			}else{
				$('.ewf-mh-icon-list', field).slideDown('fast');	//.addClass('mh-icon-remove');
			}
			
		});
		
		
		$('.mh-field-icon .ewf-mh-icon-preview').click(function(){
			var field = $(this).closest('.mh-field-icon');
		
			$('.ewf-mh-icon-list', field).slideDown('fast');
		});
		
		
		$('.mh-field-icon .ewf-mh-icon-list li').click(function(){
			var field = $(this).closest('.mh-field-icon');
			
			var title = $('i', this).attr('data-title');
			var icon = $('i', this).attr('class');
			
			$('.ewf-mh-icon-preview i', field).attr('class', icon);
			$('.ewf-mh-icon-preview span', field).html(title);
			$('input' ,field).val(icon);

			$('.ewf-mh-icon-action', field).click();
			$('.ewf-mh-icon-action', field).removeClass('mh-icon-set').addClass('mh-icon-remove');
		...
		function ewf_modHeader_handleImage(){
			...
			$('#ewf-upload-header-image').click(function(event){
				...
				file_frame.on( 'select', function() {
					...
					selection.map(function( attachment) {
						...
						$.post( ajaxPath, { action:"ewf_modHeader_setImage", url:attachment.sizes.full.url, post:postID, img:attachment.id  }, function(response){
			...
			$('.ewf-mh-useMaster .toggle').on('toggle', function (e, active) {
				...
				$.post( ajaxPath, { "action":'ewf_modHeader_master_use', 'master_use':active, 'post':postID  }, function(data){
					...
					if (active){
						...
						$('.mh-field-icon').addClass('disabled');
						...
						$('.mh-field-icon').removeClass('disabled');
