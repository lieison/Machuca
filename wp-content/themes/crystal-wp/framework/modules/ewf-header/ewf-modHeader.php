...
{ 
	...
	class EWF_ModHeader {
		...
		//	Include header settings from the settings file
		//
		include_once('ewf-modHeader-settings.php');
		
		$this->settings = $modHeader_settings;
		...
		function ajax_use_master(){
			...
			if (is_array($_POST) && array_key_exists('master_use', $_POST)){ 
				...
				$var_post_id = intval($_POST['post']);
				$var_master_use = intval($_POST['master_use']);
				
				$post_modHeader_settings = $this->get_postSettings($var_post_id);
				$post_modHeader_settings['master_use'] = $var_master_use;
				
				$this->set_postSettings($post_modHeader_settings, $var_post_id);
				
				wp_send_json_success(array('title'=>$post_modHeader_settings['title'], 'master_id'=>$var_master_use, 'post'=>$var_post_id));
			}else{
				wp_send_json_error(array('master_id'=>$var_master_use, 'post'=>$var_post_id));
			}
		...
		function ajax_set_active(){
			...
			if (is_array($_POST) && array_key_exists('active', $_POST)){ 
				...
				$post_modHeader_settings = $this->get_postSettings($post_id );
				$post_modHeader_settings['active'] = $active;
				
				$result = $this->set_postSettings($post_modHeader_settings, $post_id );
				
				wp_send_json_success(array('state'=>$result, 'post'=>$post_id ) );		
			}
	...
	function meta_update(){
			...
			if (array_key_exists('_ewf_modHeader_master_id', $_POST)){
				$post_modHeader_settings['master_id'] = intval($_POST['_ewf_modHeader_master_id']);
			}
			
			if (array_key_exists('_ewf_modHeader_master_use',  $_POST)){
				$post_modHeader_settings['master_use'] = sanitize_text_field( $_POST['_ewf_modHeader_master_use'] );
			}
					
			if (array_key_exists('_ewf_modHeader_title', $_POST)){
				$post_modHeader_settings['title'] = sanitize_text_field( $_POST['_ewf_modHeader_title'] );
			}
			
			if (array_key_exists('_ewf_modHeader_title_src', $_POST)){
				$post_modHeader_settings['title_src'] = sanitize_text_field( $_POST['_ewf_modHeader_title_src'] );
			}
			
			if (array_key_exists('_ewf_modHeader_icon', $_POST)){
				$post_modHeader_settings['icon'] = sanitize_text_field( $_POST['_ewf_modHeader_icon'] );
			}			
			
			if (array_key_exists('_ewf_modHeader_background_color', $_POST)){
				$post_modHeader_settings['background_color'] = sanitize_text_field( $_POST['_ewf_modHeader_background_color'] );
			}			
			
			if (array_key_exists('_ewf_modHeader_description', $_POST)){
				$post_modHeader_settings['description'] = sanitize_text_field( $_POST['_ewf_modHeader_description'] );
			}
			
			if (array_key_exists('_ewf_modHeader_description_src', $_POST)){
				$post_modHeader_settings['description_src'] = sanitize_text_field( $_POST['_ewf_modHeader_description_src'] );
			}
