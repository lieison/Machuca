	...
	jQuery(document).ready(function($) {
		...
		//	Attach - Tiny MCE styles
			handle_tinyMCE();
			
		
		//	Attach - Menu Icons 
			handle_wordpress_menu_icons();
		...
		//	Detect composer loading state
		$(window).load(function(){
			ewf_composer_row_update();
		});		
		
		
		$(document).on( "click", '.vc_shortcode-edit-form button.vc_panel-btn-save', function(){
			ewf_composer_row_update();
		});
		...
		//	Admin - Options  restore
		//
		$('.ewf-admin-option-restore').click(function(){
			
			var modal_title = $(this).attr('data-modal-title');
			var modal_text = $(this).attr('data-modal-text');
			var modal_cancel = $(this).attr('data-modal-cancel');
			var modal_confirm = $(this).attr('data-modal-confirm');
			var modal_confirm_color = $(this).attr('data-modal-confirm-color');
		   ...
		   confirmButtonColor: modal_confirm_color,
			confirmButtonText: modal_confirm
			
			},
			function(){
				
				$('.ewf-admin-action').val("reset");
				$('.ewf-admin-option-restore').closest('form').submit();
		...
		$('.ewf-ui-columns-tabs > div').click(function(){
			...
			var parent = $(this).closest('.ewf-ui-control-columns');
			
			if ($(this).hasClass('active')){
				if ($('.ewf-ui-control-columns', parent).hasClass('expanded') === false){
					$('.ewf-ui-control-columns', parent).addClass('expanded');
				}else{
					return false;
				}
			}
			
			$('.ewf-ui-columns-tabs > div.active', parent).removeClass('active');
			$(this).addClass('active');
			
			$('.ewf-ui-columns-tab-content.active', parent).removeClass('active');
			$('.ewf-ui-columns-tab-content.'+related, parent).addClass('active');
		});
		...
		$('.ewf-ui-input-columns-size').each(function(){
			// ewf_ui_columns_size_generate(this);
			ewf_ui_columns_size_resize(this);
		});
		
		
		$(document).on('click', '.ewf-ui-column-size-editor span', function(){
			var minSpan = $(this).closest('.ewf-ui-control-columns-size').attr('data-min');
			var maxSpan = $(this).closest('.ewf-ui-control-columns-size').attr('data-max');
			var src_columns = '';
			var src_tmp = '';
			var sidebar = false;
			var sidebar_size = 0;
			...
			var control = $(this).closest('.ewf-ui-columns-size');
			...
			if ($(column_smaller).hasClass('ewf-content-size')){
				// console.log('Content resize['+column_smaller_span_new+']!');
				
				if (12 - column_smaller_span_new <= maxSpan){
					allow_resize = true;
				}
			}

			if ($(column_smaller).hasClass('ewf-sidebar-size')){
				// console.log('Sidebar resize['+column_smaller_span_new+']');
				
				if (column_smaller_span_new <= maxSpan){
					allow_resize = true;
				}
			}
			
			
			if (column_smaller_span_new >= minSpan && allow_resize){
				...
				console.log('Column cannot be resized, min or max span reached! ['+column_smaller_span_new+']');
			...
			//	
			$('.ewf-ui-column-size-editor > div', control).each(function(){
				if (src_columns === ''){
			...
			$('.ewf-ui-input-columns-size', control).val(src_columns);
		...
		function ewf_ui_columns_size_resize(element){
			var control = $(element).closest('.ewf-ui-control-columns-size');
			var columns_size = $('.ewf-ui-input-columns-size', control).val();			
			var sizes = columns_size.split(",");
			
			$('.ewf-ui-column-size-editor > div', control).each(function(index){
				$(this).attr('data-columns', sizes[index]).addClass('ewf-ml-ftcol'+sizes[index]);
		...
		function ewf_ui_columns_generate(element){
			...
			$('.ewf-ui-column-editor', control).html('');
					...
					$.post( ajaxPath, { action:"ewf_ui_setImage", image_id:attachment.id, image_size: image_size  }, function(response){
						...
						if (response.data.state == 1){
							...
							if (response.data.class != ''){
								$('.ewf-ui-image-preview', $control).addClass(response.data.class);
							}
		...
		// $('.ewf-admin-export-options').click(function(){

			// var ajaxPath = siteURL+'/wp-admin/admin-ajax.php'; 
			// $.post( ajaxPath, { action:"ewf_import_content_download" }, function(response){ });
		...
		//	Auto hide admin confirmation message
		//
		...
		//	Attach - Tiny MCE styles
		//
		function handle_tinyMCE(){
			if (typeof(tinymce) != 'undefined'){
			
				tinymce.DOM.loadCSS(themePath  + '/layout/css/mce.css,'+themePath  + '/layout/css/fontawesome/font-awesome.min.css,'+themePath  + '/layout/css/iconfontcustom/icon-font-custom.css');
			}
		}
		...
		//	Attach - Menu Icons 
		//
		function handle_wordpress_menu_icons(){
			
			
			//	Remove Icon - Click on "Remove Icon" button
			//
			$(document).on('click', '.ewf-mnu-icon-remove', function(){
				var parent = $(this).closest('.menu-item-settings');

				$('.field-icon input', parent).val('');
				$('.ewf-mnu-icon-preview i', parent).attr('class', 'no-icon');
				$('.ewf-nav-icon-list', parent).removeClass('ewf-icon-list-visible');
				return false;
			});
			
			
			//	Set Icon - Click on "Select icon" button
			//
			$(document).on('click', '.ewf-mnu-icon-set', function(){
				var parent = $(this).closest('.menu-item-settings');
				
				$('.ewf-nav-icon-list', parent).toggleClass('ewf-icon-list-visible');
				return false;
			});
			
			
			//	Set Icon once you click on it and slide up the list
			//
			$(document).on('click', '.ewf-page-nav-icon li', function(){
				var list = $(this).closest('ul');
				var value = $('i', this).attr('class');
				var parent = $(this).closest('.menu-item-settings');
				
				$('.active', list).removeClass('active');
				$(this).addClass('active');
				
				$('.field-icon input', parent).val(value);
				$('.ewf-mnu-icon-preview i', parent).attr('class', value);
				$('.ewf-mnu-icon-set', parent).click();
			});
		...
		//	Helper function to help serialize EWF UI Options
		//
