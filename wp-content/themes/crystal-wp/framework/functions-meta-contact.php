<?php
	...
	function ewf_pgContact_metaSource() {
			...
			if (array_key_exists('_ewf-contact-address', $contact_meta)){
				$contact_address = esc_html($contact_meta["_ewf-contact-address"][0]);
				}
			
			if (array_key_exists('_ewf-contact-zoom', $contact_meta)){			
				$contact_zoom = esc_html($contact_meta["_ewf-contact-zoom"][0]);
				}
			
			if (array_key_exists('_ewf-contact-caption', $contact_meta)){			
				$contact_caption = esc_html($contact_meta["_ewf-contact-caption"][0]);
				}
	...
	function ewf_pgContact_metaUpdate() {
		...
		if (is_object($post)){
			...
			$contact_address = sanitize_text_field( $_POST["_ewf-contact-address"] );
			$contact_zoom = $_POST["_ewf-contact-zoom"];
			$contact_caption = $_POST["_ewf-contact-caption"];
			
			if (array_key_exists('_ewf-contact-address', $_POST)){
				update_post_meta($post->ID, "_ewf-contact-address"	, $contact_address );
				update_post_meta($post->ID, "_ewf-contact-zoom"		, $contact_zoom );
				update_post_meta($post->ID, "_ewf-contact-caption"	, $contact_caption );
			}
		}
	}
