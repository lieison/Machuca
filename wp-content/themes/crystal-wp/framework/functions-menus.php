<?php
	...
	function ewf_nav_meta_update($menu_id, $menu_item_db_id, $args ) {
		...
		//
		if ( array_key_exists('menu-item-mega-menu', $_REQUEST) && is_array($_REQUEST['menu-item-mega-menu'])  ) {
	...
	class EWF_Walker_Nav_Backend extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$classes = array(
				...
				'menu-item-edit-' . ( ( isset( $_GET['edit-menu-item'] ) && $item_id == intval($_GET['edit-menu-item']) ) ? 'active' : 'inactive'),
			);
					...
					<dt class="menu-item-handle">
						...
						<span class="item-controls">
							...
							<span class="item-order hide-if-js">
								...
								?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', EWF_SETUP_THEME_DOMAIN); ?>">&#8593;</abbr></a>
								|
								<a href="<?php
									echo wp_nonce_url(
										add_query_arg(
								...
								?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', EWF_SETUP_THEME_DOMAIN); ?>">&#8595;</abbr></a>
							</span>
							<a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e('Edit Menu Item', EWF_SETUP_THEME_DOMAIN); ?>" href="<?php
								echo ( isset( $_GET['edit-menu-item'] ) && $item_id == intval($_GET['edit-menu-item']) ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
